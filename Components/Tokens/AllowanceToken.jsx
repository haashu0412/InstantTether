...
const AllowanceToken = ({ setOpenComponent, setOpenSideBar }) => {
  const { READT_TOKEN, ALLOWANCE_TOKEN } = useStateContext();
  ...
  const CALLING_ALLOWANCE = async (token, allowance) => {
    const response = await ALLOWANCE_TOKEN(token, allowance);
    setAllowanceBalance(response);
  };
  return (
            ...
            <div className="justify-center flex py-2 sm:py-3">
              <div className="!bg-inherit md:!bg-default w-full rounded-2xl sm:max-w-md py-4 sm:px-3 md:px-8 sm:py-8">
                ...
                <div className="justify-center text-center w-full mt-10">
                  <button
                    onClick={() => CALLING_ALLOWANCE(token, allowance)}
